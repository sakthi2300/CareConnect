...
@Component({
    ...
    <div class="register-page">
      <div class="register-card fade-in">
        <div class="card-header">
          <div class="logo-wrap staff"><mat-icon>medical_services</mat-icon></div>
          <h1>Register as Medical Staff</h1>
          <p class="subtitle">Join CareConnect as a Doctor or Nurse</p>
        </div>
        ...
        <form (ngSubmit)="onSubmit()" #form="ngForm">
          ...
          <mat-form-field appearance="outline" class="full-width">
            ...
            <select matNativeControl name="staffType" [(ngModel)]="data.staffType" required>
          ...
          <div class="row">
            <mat-form-field appearance="outline"><mat-label>Specialization</mat-label>
              <input matInput name="specialization" [(ngModel)]="data.specialization">
            </mat-form-field>
            <mat-form-field appearance="outline"><mat-label>License Number</mat-label>
              <input matInput name="licenseNumber" [(ngModel)]="data.licenseNumber">
            </mat-form-field>
          </div>
          <div class="row">
            ...
            <mat-form-field appearance="outline"><mat-label>Experience (years)</mat-label>
              <input matInput type="number" name="yearsExperience" [(ngModel)]="data.yearsExperience" min="0">
            </mat-form-field>
          </div>
          <mat-form-field appearance="outline" class="full-width">
  ...
  styles: [`
    ...
    .logo-wrap.staff { background:#F3E5F5; }
    .logo-wrap mat-icon { font-size:24px; width:24px; height:24px; color:#7B1FA2; }
...
export class StaffRegisterComponent {
  ...
  onSubmit(): void {
    ...
    this.auth.registerStaff(this.data).subscribe({
      next: () => this.router.navigate(['/staff']),
      error: err => {
        this.loading = false;
        this.error = err.status === 0 ? 'Cannot reach server.' : (err.error?.error || err.error?.message || 'Registration failed.');
      }
    });
  }
}
